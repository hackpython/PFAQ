...
## `待审阅` 3.问题：使用CIFAR-10彩色图片训练出现输出数据维度错误
...
## `待审阅` 5.问题：在使用预测模型预测图片的时候出现in_dims[1]:32 != filter_dims[1] * groups:3错误
...
## `待审阅` 6.问题：使用预测模型预测图片时出现输出数据维度错误
