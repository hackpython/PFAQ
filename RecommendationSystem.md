...
## `待审阅` 1.问题：使用ml-1m数据集数据在训练时出现非法指令
...
## `待审阅` 2.问题：在使用ml-1m数据集训练个性化推荐模型的时候出现输入数据维度不相同错误
...
## `待审阅` 3.问题：在训练用的特征和电影特征之间的分数是出现张量类型错误
...
## `待审阅` 4.问题：在使用ml-1m数据集训练模型是出现序列数字元素的错误
...
## `待审阅` 6.问题：在使用ml-1m数据集做分类是出现非法指令错误
　
 + 关键字：`非法指令`，`分类`
