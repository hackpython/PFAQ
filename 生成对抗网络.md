...
## `已审核`2.问题：使用的Fluid构建了GAN，训练了多轮后，发现输出的损失没有变化？
...
+ 问题描述：使用的Fluid构建了GAN，训练了多轮后，发现输出的损失没有变化？损失变化如下，可以看出，几乎没有什么变动。
...
## `已审核`3.问题：Tensor not initialized yet when Tensor::type() is called
...
## `已审核`6.问题：DataType of Paddle Op mul must be the same.
...
## `已审核`7.问题： 通过Fuild使用cycleGAN后，运行报错
...
Traceback (most recent call last):
  ...
  File "/Users/jizhi/gan/cycle_gan/trainer.py", line 15, in __init__
    self.fake_B = build_generator_resnet_9blocks(input_A, name="g_A")
  File "/Users/jizhi/gan/cycle_gan/model.py", line 18, in build_generator_resnet_9blocks
    o_c2 = conv2d(o_c1, 64, 3, 2, 0.02, "SAME", name + "_c2")
  File "/Users/jizhi/gan/cycle_gan/layers.py", line 96, in conv2d
    offsets=(0, 0, 1, 1))
  File "/Users/jizhi/anaconda3/envs/paddle/lib/python3.5/site-packages/paddle/fluid/layers/nn.py", line 6102, in crop
...
## `已审核`8.问题：参考models中的GAN代码编写GAN，运行出现下面错误
...
## `已审核`9.问题：通过Fluid编写StarGAN，完全参考models中CycleGAN的代码，报错

+ 版本号：`1.0.1`

+ 标签：`GAN` `unsupported operand`
...
Traceback (most recent call last):
  ...
  File "train.py", line 56, in train
    g_A_trainer = GATrainer(input_A, input_B)
  File "/Users/jizhi/gan/StarGAN/trainer.py", line 15, in __init__
    self.fake_B = build_generator_resnet_9blocks(input_A, name="g_A")
  File "/Users/jizhi/gan/StarGAN/model.py", line 18, in build_generator_resnet_9blocks
    o_c2 = conv2d(o_c1, 64, 3, 2, 0.02, "SAME", name + "_c2")
  File "/Users/jizhi/gan/StarGAN/layers.py", line 63, in conv2d
    filter_size)
  File "/Users/jizhi/gan/StarGAN/layers.py", line 13, in cal_padding
...
## `已审核`10.问题：D must match arity(DDim)
...
## `已审核`11.问题：create_parameter() missing 1 required positional argument: 'dtype'

+ 版本号：`1.0.1`

+ 标签：`missing x required` `GAN`
...
Traceback (most recent call last):
  ...
  File "/Users/jizhi/gan/cycle_gan/trainer.py", line 15, in __init__
    self.fake_B = build_generator_resnet_9blocks(input_A, name="g_A")
  File "/Users/jizhi/gan/cycle_gan/model.py", line 17, in build_generator_resnet_9blocks
    o_c1 = conv2d(pad_input, 32, 7, 1, 0.02, name=name + "_c1")
  File "/Users/jizhi/gan/cycle_gan/layers.py", line 98, in conv2d
    conv = instance_norm(input=conv, name=name + "_norm")
  File "/Users/jizhi/gan/cycle_gan/layers.py", line 39, in instance_norm
...
## `已审核`12.问题：Parameter shape should not be related with batch-size

+ 版本号：`1.0.1`

+ 标签：`not with batch-size` `GAN`
...
Traceback (most recent call last):
  ...
  File "/Users/jizhi/anaconda3/envs/paddle/lib/python3.5/site-packages/paddle/fluid/layer_helper.py", line 317, in create_parameter
    dtype=dtype, shape=shape, **attr._to_kwargs(with_initializer=True))
  File "/Users/jizhi/anaconda3/envs/paddle/lib/python3.5/site-packages/paddle/fluid/framework.py", line 1175, in create_parameter
    param = Parameter(global_block, *args, **kwargs)
  File "/Users/jizhi/anaconda3/envs/paddle/lib/python3.5/site-packages/paddle/fluid/framework.py", line 2009, in __init__
...
## `已审核`13.问题：missing 1 required positional argument: 'value'

+ 版本号：`1.0.1`

+ 标签：`missing required value`
...
Traceback (most recent call last):
  ...
  File "/Users/jizhi/gan/c_gan/network.py", line 98, in D_cond
    x = conv_cond_concat(image, yb)
  File "/Users/jizhi/gan/c_gan/network.py", line 91, in conv_cond_concat
...
## `已审核`14.问题：'NoneType' object has no attribute 'shape'

+ 版本号：`1.0.1`

+ 标签：`NoneType` `no attribute`  `GAN`
...
Traceback (most recent call last):
  ...
  File "c_gan.py", line 69, in train
    d_logit = D_cond(img, conditions)
  File "/Users/jizhi/gan/c_gan/network.py", line 103, in D_cond
    h1 = bn(conv(h0, df_dim + y_dim), act="leaky_relu")
  File "/Users/jizhi/gan/c_gan/network.py", line 50, in conv
    act=act)
  File "/Users/jizhi/anaconda3/envs/paddle/lib/python3.5/site-packages/paddle/fluid/nets.py", line 122, in simple_img_conv_pool
    use_cudnn=use_cudnn)
  File "/Users/jizhi/anaconda3/envs/paddle/lib/python3.5/site-packages/paddle/fluid/layers/nn.py", line 1593, in conv2d
...
## `待审核`16.问题：对抗生成网络训练没办法生成CIFAR彩色图像

+ 版本号：`1.0.1`

+ 标签：`GAN` `CIFAR`

+ 问题描述：我按照GAN生成mnist灰度图片的程序：https://github.com/yeyupiaoling/LearnPaddle2/blob/master/note6/GAN.py

修改了一些必要的参数，是能够正常训练，但是不能生成正常的图片，都是噪点。
...
def Generator(y, name="G"):
    ...
    with fluid.unique_name.guard(name + "/"):
        ...
        # 第二组转置卷积运算
        y = deconv(x=y, num_filters=3, act='sigmoid', output_size=[32, 32])
    return y
...
从提供的代码来看，生成器与判别器的结构都比较简单，相比手写体数据，cifar的数据还是相对比较复杂的，所以比较难训出效果

+ 解决方法：

可以从以下几个方面来改进

1.不以纯噪声作为生成器的输入，而是输入加噪后的训练图像作为输入，不必让生成器从零开始
2.使用WGAN-GP
3.增多生成器与判别器的网络结构，构造学习能力更强的网络

## `待审核`17.问题：Fluid如何实现Progressive Growing of GANs中模型递增的逻辑？
...
+ 问题描述：
PGGAN的模型随着训练的进行而增加，Paddle是否有想过的函数直接实现？如果没有，应该怎么实现？说一下简单思路

+ 问题分析：

PGGAN在训练的过程模型结构渐近的增加，从而确保最后生成器可以生成高清的图像，目前Fluid中并没有提供某个方法可以直接实现这个过程。

可以通过Fluid保存模型与加载模型的机制来实现，首先逻辑上，模型的代码结构是可以改变的，然后每一种模型结构训练完一遍全数据后，将其保存，接着再加载，加载完后，此时的模型结构中就存在相应的参数，此时在动态的增加模型结构，实现PGGAN。

+ 解决方法：

在实现PGGAN时，只需保存以及加载模型本身的参数则可，我们可以使用 fluid.io.save_params() 接口来进行模型参数的保存。
...
具体实现，可以参考下面TensorFlow的实现逻辑：https://github.com/zhangqianhui/progressive_growing_of_gans_tensorflow
